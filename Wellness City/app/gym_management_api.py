...
def checkIfMemberIsInCheckedTable(connection, cursor, email):
    values = [email]
    query = """SELECT * FROM tbl_checked_in WHERE email = %s"""
    cursor.execute(query, values)
    result = cursor.fetchall()
    return result
...
def updateMemberCheckIn(connection, cursor, email):
    check_in_time = str(datetime.datetime.now())
    values = [check_in_time, email]
    query = """UPDATE tbl_checked_in SET check_in_timestamp = %s, is_checked_in = 1, check_out_timestamp = NULL WHERE email = %s"""
...
def checkOutMember(connection, cursor, email):
    ...
    query = """UPDATE tbl_checked_in SET check_out_timestamp = %s, is_checked_in = 0  WHERE email = %s"""
    cursor.execute(query, values)
    connection.commit()
...
def getEnrolledClassNames(cursor, username):
    class_query = """SELECT cr.class_name FROM tbl_class_enrolled cr INNER JOIN tbl_class_schedule cs ON cs.class_name = cr.class_name and cs.location = cr.location WHERE cr.member_name=%s"""
    values = [username]

    cursor.execute(class_query, values)
    class_result = cursor.fetchall()
    print("view my class names:", class_result)
    return class_result
...
def getGymEquipments(cursor):
    query = """select equipment_name from tbl_equipments"""
    cursor.execute(query)
    result_equipments = cursor.fetchall()
    return result_equipments
...
def getLoggedHours(cursor, username, json_format=False):
    class_query = """SELECT equipment_name, num_of_minutes, DATE(date_column) as date_column, Location FROM tbl_member_log WHERE user_name=%s order by date_column DESC"""
    values = [username]
...
def pastWeekStats(cursor, username, json_format=False):
    week_query = """SELECT equipment_name, num_of_minutes, date_column, Location FROM tbl_member_log WHERE user_name=%s AND date_column >= DATE_SUB(NOW(), INTERVAL 1 WEEK) order by date_column DESC"""
    values = [username]
...
def pastMonthStats(cursor, username, json_format=False):
    month_query = """SELECT equipment_name, num_of_minutes, date_column, Location FROM tbl_member_log WHERE user_name=%s AND date_column >= DATE_SUB(NOW(), INTERVAL 1 MONTH) order by date_column DESC"""
    values = [username]
...
def ninetyDayStats(cursor, username, json_format=False):
    ninety_query = """SELECT equipment_name, num_of_minutes, date_column, Location FROM tbl_member_log WHERE user_name = %s AND date_column >= DATE_SUB(NOW(), INTERVAL 90 DAY) ORDER BY date_column DESC"""
